...
const orderHistory = [
  { id: 'ORD-3421', customer: 'Alex Johnson', address: '423 Maple St', status: 'Delivered', date: '2025-04-22', time: '14:30', service: 'Wash & Fold', amount: '$34.50' },
  { id: 'ORD-3420', customer: 'Sarah Williams', address: '128 Oak Ave', status: 'In Progress', date: '2025-04-22', time: '11:15', service: 'Dry Clean', amount: '$56.75' },
  { id: 'ORD-3419', customer: 'Michael Chen', address: '892 Pine Blvd', status: 'Scheduled', date: '2025-04-23', time: '09:00', service: 'Wash & Fold', amount: '$28.50' },
  { id: 'ORD-3418', customer: 'Emily Rodriguez', address: '742 Cedar Ln', status: 'Delivered', date: '2025-04-21', time: '16:45', service: 'Dry Clean', amount: '$42.25' },
  { id: 'ORD-3417', customer: 'David Kim', address: '356 Elm St', status: 'Delivered', date: '2025-04-21', time: '10:30', service: 'Wash & Fold', amount: '$31.00' },
  { id: 'ORD-3416', customer: 'Lisa Thompson', address: '219 Birch Ave', status: 'Cancelled', date: '2025-04-20', time: '13:00', service: 'Dry Clean', amount: '$0.00' },
  { id: 'ORD-3415', customer: 'James Wilson', address: '734 Aspen Dr', status: 'Delivered', date: '2025-04-20', time: '09:15', service: 'Wash & Fold', amount: '$27.50' },
  { id: 'ORD-3414', customer: 'Maria Garcia', address: '495 Walnut St', status: 'Delivered', date: '2025-04-19', time: '14:45', service: 'Rush Wash', amount: '$48.25' },
];
...
const OrdersPage = () => {
  ...
  const orderFlowSteps = [
    { title: 'Order Creation', description: 'Customer places order through app/website', icon: <User /> },
    { title: 'Incoming Delivery', description: 'Courier picks up laundry from customer', icon: <Truck /> },
    { title: 'Order Processing', description: 'Laundry is washed, dried, folded as ordered', icon: <Package /> },
    { title: 'Outgoing Delivery', description: 'Clean items delivered back to customer', icon: <Truck /> },
    { title: 'Order Complete', description: 'Payment processed and order completed', icon: <CheckCircle /> }
  ];
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center space-x-2">
                ...
                <div className="flex bg-gray-100 rounded-lg p-1">
                  <button 
                    className={`px-3 py-1 text-sm rounded-md transition ${activeView === 'all' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
                    onClick={() => setActiveView('all')}
                  >
                    All Orders
                  </button>
                  <button 
                    className={`px-3 py-1 text-sm rounded-md transition ${activeView === 'active' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
                    onClick={() => setActiveView('active')}
                  >
                    Active
                  </button>
                  <button 
                    className={`px-3 py-1 text-sm rounded-md transition ${activeView === 'completed' ? 'bg-white shadow' : 'hover:bg-gray-200'}`}
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
              <div className="text-sm text-gray-700">
                Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredOrders.length}</span> of <span className="font-medium">{orderHistory.length}</span> orders
              </div>
              <div className="flex items-center space-x-2">
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600">Order placed on {selectedOrder.date} at {selectedOrder.time}</p>
              </div>
                  ...
                  {orderFlowSteps.map((step, index) => {
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center relative z-10">
                        ...
                        <div className="text-xs text-gray-500 text-center max-w-xs">{step.description}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="mb-6">
              ...
              <div className="border-l-2 border-gray-200 pl-4 ml-4 space-y-6">
                <div className="relative">
                  ...
                  <div className="text-xs text-gray-500">{selectedOrder.date} at {selectedOrder.time}</div>
                ...
                {selectedOrder.status !== 'Scheduled' && (
                  <div className="relative">
                    ...
                    <div className="text-xs text-gray-500">{selectedOrder.date} at {
                      new Date(`2025-04-22T${selectedOrder.time}`).getHours() + 1 + ':' + 
                ...
                {(selectedOrder.status === 'In Progress' || selectedOrder.status === 'Delivered') && (
                  <div className="relative">
                    <div className="absolute -left-6 mt-1 h-4 w-4 rounded-full bg-blue-600"></div>
                    <div className="text-sm font-medium">Processing Started</div>
                    <div className="text-xs text-gray-500">{selectedOrder.date} at {
                      new Date(`2025-04-22T${selectedOrder.time}`).getHours() + 2 + ':' + 
                      new Date(`2025-04-22T${selectedOrder.time}`).getMinutes()
                    }</div>
                ...
                {selectedOrder.status === 'Delivered' && (
                  <>
                    <div className="relative">
                      ...
                      <div className="text-xs text-gray-500">{selectedOrder.date} at {
                        new Date(`2025-04-22T${selectedOrder.time}`).getHours() + 4 + ':' + 
                        new Date(`2025-04-22T${selectedOrder.time}`).getMinutes()
                      }</div>
                      <div className="text-sm mt-1">Processing complete, items ready for delivery</div>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <div className="text-xs text-gray-500">{selectedOrder.date} at {
                        new Date(`2025-04-22T${selectedOrder.time}`).getHours() + 5 + ':' + 
                        new Date(`2025-04-22T${selectedOrder.time}`).getMinutes()
                      }</div>
